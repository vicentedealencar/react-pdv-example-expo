import React from 'react'
import { Text, View, Alert } from 'react-native'
import { Cart } from 'react-pdv'
import BasicButton from './Button.basic.example'

const RegularButton = ({ style, ...otherProps }) => (
  <BasicButton {...otherProps} style={{ marginTop: 10, ...style }} />
)

const SubmitButton = ({ style, ...otherProps }) => (
  <RegularButton
    {...otherProps}
    style={{ backgroundColor: '#007777', ...style }}
  >
    Comprar
  </RegularButton>
)

const MonoText = ({ style, ...otherProps }) => (
  <Text
    style={{
      fontFamily: 'space-mono',
      marginLeft: 10,
      marginRight: 10,
      textAlign: 'center',
      ...style
    }}
    {...otherProps}
  />
)

const Title = ({ children, style, ...otherProps }) => (
  <Text
    style={{
      fontSize: 30,
      marginTop: 10,
      marginBottom: 10,
      textAlign: 'center',
      ...style
    }}
    {...otherProps}
  >
    {`Total: ${children}`}
  </Text>
)

const basicCart = {
  items: [
    {
      id: 'prod1',
      name: 'prod1',
      description: 'lorem ipsum dolor flanders',
      amount: 1,
      price: 123.45
    },
    {
      id: 'prod2',
      name: 'prod2',
      description: 'lorem ipsum dolor flanders',
      amount: 6,
      price: 678.9
    }
  ]
}

export default class extends React.Component {
  state = {
    cart: basicCart
  }

  updateCartItem = item =>
    this.setState(state => ({
      cart: {
        ...state.cart,
        items: state.cart.items.map(it => (it.id !== item.id ? it : item))
      }
    }))

  render() {
    return (
      <Cart
        cart={this.state.cart}
        updateCartItem={this.updateCartItem}
        Title={Title}
        Text={MonoText}
        Button={RegularButton}
        SubmitButton={SubmitButton}
        onSubmit={cart => Alert.alert('cart', JSON.stringify(cart, null, 2))}
        {...this.props}
        View={View}
        Container={View}
      />
    )
  }
}
